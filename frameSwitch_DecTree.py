#Implements the Decision Tree classifier for the frame switch prediction
import numpy as np
...
def runModel(matrix, label, num_train, index_pred):
  ...
  fit=clf.fit(matrix[:num_train,:], label[:num_train])
...
print(f'Time to train Decision Tree model ={end-start} s')
