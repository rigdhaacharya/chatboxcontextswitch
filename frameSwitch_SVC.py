#Implements the SVM classifier for the frame switch prediction
import numpy as np
...
#helper function for the feature selection by wrapper method
def runModel(matrix, label, num_train, index_pred):
  classifier=OneVsRestClassifier(SVC(kernel='poly', class_weight='balanced'))
...
clf = OneVsRestClassifier(SVC(kernel='poly', class_weight='balanced'))
start=time.time()
fit=clf.fit(dat_train1[:num_train,bestFeatures], label_array[:num_train])
end=time.time()
print(f'Time to train SVC model ={end-start} s')
